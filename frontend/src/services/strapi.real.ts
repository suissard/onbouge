...
const BASE_URL = 'http://192.168.0.2:1337';
...
const strapiService = {
  ...
  findOne(collection: string, id: string | number, queryParams?: any) {
    return strapiClient.collection(collection).findOne(id, queryParams);
  },
  ...
  update(collection: string, id: string | number, data: any, queryParams?: any) {
    return strapiClient.collection(collection).update(id, data, queryParams);
  },

  async login(credentials: any) {
    const response = await strapiClient.fetch('/api/auth/local', {
      method: 'POST',
      headers: {
  ...
  async register(userInfo: any) {
    const response = await strapiClient.fetch('/api/auth/local/register', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
