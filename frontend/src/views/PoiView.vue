    ...
    <v-card v-if="poi">
      ...
      <v-card-text>
        ...
        <div v-if="poi.sports && poi.sports.length > 0">
          ...
          <v-chip-group>
            <v-chip v-for="sport in poi.sports" :key="sport.id" :to="`/sport/view/${sport.id}`">{{ sport.title }}</v-chip>
          </v-chip-group>
        </div>
        <div v-if="poi.events && poi.events.length > 0">
          <h3 class="text-h6 mt-4">Events</h3>
          <v-chip-group>
            <v-chip v-for="event in poi.events" :key="event.id" :to="`/event/view/${event.id}`">{{ event.title }}</v-chip>
          </v-chip-group>
        </div>
      </v-card-text>
    </v-card>
